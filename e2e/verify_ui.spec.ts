...
test('verify home page and demo pages', async ({ page }) => {
  ...
  // Verify Hero Section
  await expect(page.getByRole('heading', { name: 'Welcome to the SvelteKit Demo Application' })).toBeVisible();

  // Verify Cards
  await expect(page.getByRole('link', { name: 'Authentication Secure login' })).toBeVisible();
  await expect(page.getByRole('link', { name: 'Localization Internationalization (i18n) using Paraglide JS.' })).toBeVisible();
